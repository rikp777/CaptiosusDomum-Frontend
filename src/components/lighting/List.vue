<template>
  <div class="card">
    <div class="card-body">
      <h3 class="card-title">Lighting Overview</h3>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        items: 'lightings',
        isLoading: 'lightingIsLoading',
        error: 'lightingError'
      })
    },
    methods: {
      refreshList() {
        console.log("Lighting Module - Refresh List")
        this.$store.dispatch('getAllLightings')
      },
      updateMode(item) {
        console.log("Lighting Module - UpdateMode")
        this.$emit('updateMode', item)
      },
      deleteMode(item) {
        console.log("Lighting Module - DeleteMode")
    ...
    mounted() {
      ...
      if(this.debug){
        ...
        this.$store.commit('setLightings', items)
      }
    }
  }
</script>
